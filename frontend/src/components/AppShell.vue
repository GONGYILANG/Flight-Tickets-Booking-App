...
<template>
  <div class="flex min-h-dvh flex-col">
    <header class="bg-slate-950 text-white dark:bg-neutral-900 dark:text-neutral-100">
      <div class="mx-auto flex h-16 max-w-screen-2xl items-stretch gap-3 px-4 sm:px-8">
        <nav class="flex min-w-0 gap-1 overflow-x-auto" aria-label="Main navigation">
          <RouterLink
            ...
            :key="link.to"
            :to="link.to"
            class="inline-flex shrink-0 items-center gap-2 border-b-2 px-3 text-sm
            font-medium whitespace-nowrap transition-colors hover:bg-white/5
            focus-visible:outline-2 focus-visible:outline-teal-300"
            :class="
        ...
        <button
          type="button"
          class="ml-auto flex shrink-0 items-center px-2 text-sm hover:text-teal-200"
        ...
        <RouterLink
          class="flex shrink-0 items-center gap-2 text-sm hover:text-teal-200"
          to="/profile"
